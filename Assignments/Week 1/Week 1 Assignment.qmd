...
```{r}
  ...
  train %>% 
    model(
      STL(box_cox(value, lambda) ~ trend(window = 33) + season(window = 21), robust = TRUE)
    ) %>% 
    components() %>% 
    autoplot() +
    theme_bw()
  ...
  train %>% 
    model(
      STL(value ~ trend(window = 33) + season(window = 21), robust = TRUE)
    ) %>% 
    components() %>% 
    autoplot() +
    theme_bw()
...
```{r}
  ...
  cv.results <- fx.cv %>% 
    accuracy(test.cv, by = c(".id", "period", ".model")) 

  cv.results %>% 
    ggplot(aes(x = period, y = RMSE, color = .model)) +
    facet_grid(. ~ .model) +
