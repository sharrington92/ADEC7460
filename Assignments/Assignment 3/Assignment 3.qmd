...
To evaluate the cross validated error
...
Cross validation has found the model with 6 nodes to minimize the median RMSE, however the variance on that error is quite large. With that consideration, it may be best to choose the model with 7  nodes as the median RMSE is comparable, but the variance is much lower.
