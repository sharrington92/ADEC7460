...
#| fig-width: 12
 ...
 train %>% 
  ...
  pivot_longer(-c(any_of(c(vars.id, "days_in_week", "is_missing"))), names_to = "variable") %>% 
...
* San Jose models were trained on `r train.start.sj%--%(valid.start.sj-weeks(1))` with the validation set covering `r valid.start.sj%--%(test.start.sj-weeks(1))`.
...
> Iquitos has a lower mean absolute error than San Jose, however, this is likely a result of having an average of 7.54 weekly cases to San Jose's average of 34.1 during the training set. 

> Surprisingly, the Seasonal Mean Plus Trend approach (Model 1) outperforms the ARIMAX approach (Model 2) for both cities, and the Boosted Prophet approach for San Jose. 

> The Prophet with Boosted Trees performs best for Iquitos.

> The best model likely is the ensemble model as it's only slightly edged out for Iquitos but significantly better for San Jose. 
...
* San Jose models were trained on the full training set, `r train.start.sj%--%(valid.start.sj-weeks(1))`, with the test set covering `r test.start.sj%--%(test.end.sj)`.
