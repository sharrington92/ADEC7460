...
fx3.sj_trans <- calibration_table_sj %>%
  modeltime_forecast(
    ...
    # actual_data = as_tibble(train.all) %>% filter(city == "sj", year >= 2008)
  )
...
fx3.test.iq <- modeltime_table(fit3.iq_final) %>%
  ...
  fn_standardize_fx(., 3, "iq")


fx.test.iq <- bind_rows(fx1.test.iq, fx2.test.iq, fx3.iq)
